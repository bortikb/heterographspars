...
//	int sketchesMemoryInMB; 
	int showPairsGroundTruth;
	int showTrianglesGroundTruth;
};
...
class BayesLSH{
	...
	float processPair_cos(const int, const int, double*);
	float processPair_jac(const int, const int, double*);
	float (BayesLSH::*processPair)(const int, const int, double*);
	...
	{
		// dfuhry
		if ((pruned_f = fopen("pruned_pairs", "wb")) == NULL)
		{
			perror("fopen pruned_pairs");
			exit(1);
		}
		if ((concentrated_f = fopen("concentrated_pairs", "wb")) == NULL)
		{
			perror("fopen concentrated_pairs");
			exit(1);
		}
		if ((simpruned_f = fopen("simpruned_pairs", "wb")) == NULL)
		{
			perror("fopen simpruned_pairs");
			exit(1);
		}
		if ((triangles_f = fopen("triangles", "wb")) == NULL)
		{
			perror("fopen triangles");
			exit(1);
		}
	}
	~BayesLSH();
...
//	Matrix* processIndexes();
	...
	void (LSH::*processPair)(const int, const int, Matrix*, Matrix*,
	RectMatrix*);

	void processPair_blshWrapper(const int, const int, Matrix*, Matrix*,
	RectMatrix*);
	void processPair_jac_exact(const int, const int, Matrix*, Matrix*,
	RectMatrix*);
	void processPair_cos_exact(const int, const int, Matrix*, Matrix*,
	RectMatrix*);
	void processPair_jac(const int, const int, Matrix*, Matrix*,
	RectMatrix*);
	void processPair_cos(const int, const int, Matrix*, Matrix*,
	RectMatrix*);
...
class AllPairsRealCosine{
	...
	*invIndex_starts, *invIndex_ends;
	...
	void addPairsForVertex(const int, Matrix*, Matrix*, uint8_t*);
	void pruneAddPairsForVertex(const int, Matrix*, Matrix*, uint8_t*,int*);
	void doublePruneAddPairsForVertex(const int, Matrix*, Matrix*,
	uint8_t*, int*, float*);
...
//	Matrix *fvs, *reordered ;
	...
	opts(opts){
		/*
		if ((variance_f = fopen("variance", "wb")) == NULL)
		{
			perror("fopen variance");
			exit(1);
		}
		*/
	}
...
class AllPairsBinary{
	...
	*ascOutDegreeOrder;
	...
	Matrix* simPairs();
	void pruneAddPairsForVertex(const int, Matrix*, Matrix*, uint8_t*,
	int*);
	void addPairsForVertex(const int, Matrix*, Matrix*, uint8_t*);
	void doublePruneAddPairsForVertex(int, Matrix*, Matrix*, uint8_t*,
	int*, float*);
	...
	void processPair_blshWrapper(const int, const int, Matrix*, Matrix*);
	void processPair_noBlsh(const int, const int, Matrix*, Matrix*);
